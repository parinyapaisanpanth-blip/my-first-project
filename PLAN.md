...
> สถานะ: เฟส 0–5 done — project complete · อัปเดต 2026-05-17 · Vercel git-connected → auto-deploy on push ทำงานแล้ว
...
| 1 — ข่าวรายวัน | scout-news + .github/workflows/daily.yml + expiry cleanup + auto-commit — **done** (TSM 13F-dump noise แก้แล้วในเฟส 5: headline-only + dump filter) | 0 |
...
| 5 — Hardening | headline-only relevance + 13F-dump filter, `fetchRetry` (Finnhub×2 + FX) backoff 1s/2s, actions checkout/setup-node v5 + Node24, การ์ดเงินสดเศษใน showcase, FX fallback → currency-api jsdelivr (no key), `concurrency: daily-news` กัน run ซ้อน — **done** | 1-4 |
